-- Crypt-MM2-Legit | Aimbot + Triggerbot with delay setting
...
RunService.RenderStepped:Connect(function(dt)
    ...
    local trigDelay = _G.TriggerDelay    or 0.18  -- tweakable delay
    ...
    -- Triggerbot — whole body via HumanoidRootPart distance, tweakable delay
    if trigOn then
        trigCD = trigCD - dt
        if trigCD <= 0 then
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                ...
                if (Vector2.new(sp.X,sp.Y) - center).Magnitude <= trigFOV then
                    ...
                    trigCD = trigDelay + math.random() * 0.05; break
                end
            end
        end
    end
